...
| Design-system theme fan (`.doco-fan`) | `assets/case-doconomy.css`; markup in `_cases/doconomy.html` | Reworked from Figma Motion `107:44395`, after the daisyUI theme preview. The same design-system board rendered three times - cream `#f7f6f2`, white, dark `#20201d` - stacked and sliced into diagonal bands that fan open and closed, so all three themes are on screen at once. **A deliberate departure from the Figma timeline.** The original swept one flat mask off to the right, but it stops at `translate: 1237px` against a 1272px board, so it never cleared: roughly the right tenth of the previous theme stayed behind, slicing through the form fields. *Geometry* - the bands radiate from a pivot just below-left of the board (`-120, 780` in board px), so like daisyUI they converge off-canvas instead of running parallel or meeting in the middle. Each band is a sector on a **square** wrapper centred on that pivot; square because `clip-path` percentages resolve against width and height separately and would skew a sector as it turns on the 1272x502 oblong. The wrapper is 3300 board px, enough to still cover the far corner (1596 away) once turned. *Motion* - the wrapper carries the turn while the board inside counter-turns by the same amount about the same pivot, so the UI stays upright and pixel-registered with the base board (measured at 0,0 for both wedges). The sectors are nested, both starting from the same `-90deg` edge with the narrow dark one over the wider white one, so the visible bands are the gaps between their outer edges; swinging those edges by different amounts (dark 7deg, white 17deg) widens and narrows the bands, which is what makes it fan rather than slide. At rest the dark is a small corner and the light themes take the body - daisyUI's balance - and at full spread the dark and white bands have grown and pushed the base board right. Both are plain rotations, so it composites instead of repainting a clip every frame. Boards are 3x PNGs (3817x1506 for a 1272px slot), sharp on retina past the 1400 canvas | `--doco-fan` (11s, `alternate` `ease-in-out`) - ours, not Figma's | `animation: none` - the bands hold their authored angles, which is the daisyUI still |
